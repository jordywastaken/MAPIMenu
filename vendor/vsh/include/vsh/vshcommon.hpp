...
namespace vsh
{
	...
	static void ShowNavigationMessage(wchar_t const* message) 
	{
		...
		static uint64_t previousNavMsgTime = 0;
		...
		{
			uint64_t currentTime = sys_time_get_system_time() / 1000;

			if (previousNavMsg) 
			{
				if (currentTime - previousNavMsgTime < 7500)
			...
			previousNavMsgTime = currentTime;
		}
	}
}